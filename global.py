	...
	for i,seq1 in enumerate(s2):
			for j,seq2 in enumerate(s1):
					if i==0 or j==0:
						...
						print score_mx[i][j],score, s1[j],s2[i]
						...
						if max(score_mx[i-1][j-1]+score,score_mx[i-1][j]-5,score_mx[i][j-1]-5)==score_mx[i-1][j-1]+score:# and MATCH==False:
							traceback_mx[i][j]=3
						elif max(score_mx[i-1][j-1]+score,score_mx[i-1][j]-5,score_mx[i][j-1]-5)==score_mx[i][j-1]-5:
							traceback_mx[i][j]=1
						elif max(score_mx[i-1][j-1]+score,score_mx[i-1][j]-5,score_mx[i][j-1]-5)==score_mx[i-1][j]-5:
							traceback_mx[i][j]=2
